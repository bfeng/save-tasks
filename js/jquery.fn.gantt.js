...
jQuery.fn.gantt = function (options) {
	...
	var create = function(jQuerythis) {
		...
		jQuery.getJSON(options.source, function(rData) {
			...
			jQuery('.fn-gantt .fn-gantt-dataPanel').css({'margin-left': hPosition+'px'});
		...
		var createContainer = function() {
			...
			var ganttLeftPanel = jQuery('<div class="leftPanel"/>')
				.append(jQuery('<div class="fn-gantt-row spacer"/>')
			...
			jQuery.each(data, function(i, entry) {
				...
				{
					ganttLeftPanel
						.append(jQuery('<div class="fn-gantt-row name"/>').append(jQuery('<span class="label"/>').html(entry.name)))
						.append(jQuery('<div class="fn-gantt-row desc"/>').append(jQuery('<span class="label"/>').html(entry.desc)));
				}
			});
		...
		var createDataContainer = function() {
			...
			var years = jQuery("<div class='fn-gantt-row'/>");
			var year = range[0].getFullYear();
			var daysInYear = 0;

			var months = jQuery("<div class='fn-gantt-row'/>");
			var month = range[0].getMonth();
			var daysInMonth = 0;
			
			var days = jQuery('<div class="fn-gantt-row"/>');
			var dow = jQuery('<div class="fn-gantt-row"/>');
			...
			jQuery.each(range, function(i, rday) {
				if (rday.getFullYear() != year) {
					years.append(jQuery('<div class="fn-gantt-row header year"/>')
				...
				if (rday.getMonth() != month) {
					months.append(jQuery('<div class="fn-gantt-row header month"/>')
				...
				days.append(jQuery('<div class="fn-gantt-row day '+day_class+'" />').html(rday.getDate()));
				dow.append(jQuery('<div class="fn-gantt-row day '+day_class+'" />').html( options.dow[rday.getDay()] ));
			...
			var dataPanel = jQuery("<div class='fn-gantt-dataPanel'/>").css("width", range.length*tools.getCellSize()+"px");
			...
			jQuery.each(data, function(i, entry) {
				if (i >= pageNum*options.itemsPerPage && i < (pageNum*options.itemsPerPage+options.itemsPerPage))
				{
					var dRow = jQuery('<div class="fn-gantt-row">');
					jQuery.each(range, function(j, day) {
						var todayCls = (today - day == 0) ? ' today' : day.getDay() == 6 ? ' sa' : (day.getDay() == 0 ? ' sn' : '');
						dRow.append(jQuery('<div class="fn-gantt-row day' + todayCls + '" id="d'+i+'-'+ tools.genId(day.getTime())+'" />')
						.html(day));
					});
		...
		navigateTo = function(val) {
			switch (val) {
				case 'begin':
					jQuery('.fn-gantt .fn-gantt-dataPanel').animate({
						'margin-left': '0px'
					}, 'fast', function() {repositionLabel();});
					break;
				case 'end':
					var mLeft = jQuery('.fn-gantt .fn-gantt-dataPanel').width() - jQuery('.fn-gantt .rightPanel').width();
					jQuery('.fn-gantt .fn-gantt-dataPanel').animate({
						'margin-left': '-' + mLeft + 'px'
					}, 'fast', function() {repositionLabel();});
					break;
				default:
					var max_left = (jQuery('.fn-gantt .fn-gantt-dataPanel').width() - jQuery('.fn-gantt .rightPanel').width()) * -1;
					var cur_marg = jQuery('.fn-gantt .fn-gantt-dataPanel').css('margin-left').replace('px','');
		...
		navigatePage = function(val) {
			...
			{
				pageNum += val;
				hPosition = jQuery('.fn-gantt .fn-gantt-dataPanel').css('margin-left').replace('px','');
				jQuerythis.empty();
				create(jQuerythis);
			}
		};
		...
		repositionLabel = function() {
			jQuery('.fn-gantt .fn-gantt-dataPanel').stop();
	...
	var tools = new function() {
		...
		this.getCellSize = function() {
			...
			{
				jQuery("body").append(
					jQuery("<div style='display: none; position: absolute;' class='fn-gantt' id='measureCellWidth'><div class='fn-gantt-row'></div></div>")
				);
				_getCellSize = jQuery('#measureCellWidth .fn-gantt-row').height();
		...
		this.getProgressBarMargin = function() {
			...
			{
				jQuery("body").append(
					jQuery("<div style='display: none; position: absolute;' id='measureBarWidth' ><div class='fn-gantt'><div class='rightPanel'><div class='fn-gantt-dataPanel'><div class='fn-gantt-row day'><div class='bar' /></div></div></div></div></div>")
				);
